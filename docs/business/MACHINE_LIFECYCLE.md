...
| Delivered | Automatic, same transaction as NTR creation | Warranty Active | NTR orchestration (writes `vehicles.delivery_date`) | System (automatic) | Delivery Date must come from NTR only - **currently violated**, see `docs/architecture/BUSINESS_INVARIANTS.md` |
