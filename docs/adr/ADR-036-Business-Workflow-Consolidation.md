...
**Proposed. Audit only.** No code, schema, migration, API, or UI
change. Full findings in `docs/architecture/
BUSINESS_WORKFLOW_CONSOLIDATION_AUDIT.md` and the new companion
`docs/architecture/BUSINESS_INVARIANTS.md`; this ADR records the
audit's existence and headline conclusion, not the detail.
