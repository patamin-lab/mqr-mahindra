...
import { createInspectionAttachmentResourceProvider } from '../utils/inspectionAttachmentResourceProvider';
...
/** PDI adapter for shared image primitives. Non-image evidence keeps the
 * legacy open/download behavior without reintroducing an image viewer. */
export default function InspectionEvidenceGallery({
  items: initialItems,
  emptyMessage,
  unavailableMessage,
  openLabel,
  ...
  labels,
  navigationLabels,
}: InspectionEvidenceGalleryProps) {
  const [items, setItems] = useState(initialItems);
  const [open, setOpen] = useState(false);
  const [index, setIndex] = useState(0);
  const provider = useRef(createInspectionAttachmentResourceProvider(initialItems)).current;
    ...
    void Promise.all(
      initialItems.map(async (item) => {
        ...
        try {
          ...
          return {
            id: item.id,
            item: { ...item, resourceState: snapshot.state, error: snapshot.error ?? null },
          };
        }
      })
    ).then((resolved) => {
  ...
  if (items.length === 0) return <p className="py-4 text-center text-sm text-gray-400">{emptyMessage}</p>;
