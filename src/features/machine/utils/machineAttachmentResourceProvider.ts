...
/** Machine Passport presentation adapter. Existing attachment API owns
 * session, permission, storage, and signed URL decisions. */
export function createMachineAttachmentResourceProvider(initialItems: readonly ImageItem[] = []): InMemoryAttachmentResourceProvider {
  ...
  return new InMemoryAttachmentResourceProvider(async ({ attachmentId, previous }) => {
    ...
    return createImageItem({
      ...
      alt: prior?.alt ?? 'Machine attachment',
      label: prior?.label,
