...
import { createMachineAttachmentResourceProvider } from '../utils/machineAttachmentResourceProvider';

interface MachineDocumentsGalleryProps {
  items: ImageItem[];
  emptyMessage: string;
...
/** Machine Passport adapter. Shared image primitives own image rendering,
 * transforms, resource state, and retry. Non-image records keep open/download
 * compatibility without adding another image viewer. */
export default function MachineDocumentsGallery({
  items: initialItems,
  emptyMessage,
  unavailableMessage,
  openLabel,
  ...
  labels,
  navigationLabels,
}: MachineDocumentsGalleryProps) {
  const [items, setItems] = useState(initialItems);
  const [open, setOpen] = useState(false);
  const [index, setIndex] = useState(0);
  const provider = useRef(createMachineAttachmentResourceProvider(initialItems)).current;
