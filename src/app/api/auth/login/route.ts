...
/** v3.0.1 reliability patch: every `logAuthEvent`/`sendAccountLockedEmail`/
 *  `upgradePasswordHash` call below is now awaited (via `ensureCompletion`,
 *  which never throws and never changes the response) instead of
 *  fire-and-forget - see `forgot-password/route.ts` for the production
 *  incident that motivated this across the whole Authentication Platform. */
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (user) {
      ...
      if (lock.isLocked) {
        ...
        await ensureCompletion(
          logAuthEvent('LOGIN_FAILED', { username, userId: user.id, ipAddress, userAgent: device, metadata: { reason: 'locked' } }),
          { task: 'logAuthEvent:LOGIN_FAILED', userId: user.id }
        );
        return NextResponse.json(
    ...
    if (!user || !passwordOk || user.active === false) {
      if (user) {
        ...
        if (lockResult.isLocked) {
          await ensureCompletion(
            logAuthEvent('ACCOUNT_LOCKED', { username, userId: user.id, ipAddress, userAgent: device }),
            { task: 'logAuthEvent:ACCOUNT_LOCKED', userId: user.id }
          );
          if (user.email) {
            await ensureCompletion(sendAccountLockedEmail(user.email, LOCKOUT_MINUTES, user.id), {
              task: 'sendAccountLockedEmail',
              userId: user.id,
            });
          }
        }
      ...
      await insertLoginLog({ username, role: user?.role ?? '', action: 'เข้าสู่ระบบ', device, result: 'fail' });
      await ensureCompletion(
        logAuthEvent('LOGIN_FAILED', { username, userId: user?.id ?? null, ipAddress, userAgent: device }),
        { task: 'logAuthEvent:LOGIN_FAILED', userId: user?.id ?? null }
      );
    ...
    if (user.password_algo !== 'scrypt') {
      ...
      await ensureCompletion(upgradePasswordHash(user.id, newHash, salt), {
        task: 'upgradePasswordHash',
        userId: user.id,
      });
    }
    ...
    await ensureCompletion(
      logAuthEvent('LOGIN_SUCCESS', { username: user.username, userId: user.id, ipAddress, userAgent: device }),
      { task: 'logAuthEvent:LOGIN_SUCCESS', userId: user.id }
    );
