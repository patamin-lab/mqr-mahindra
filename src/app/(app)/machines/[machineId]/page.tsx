...
/**
 * Machine Digital Passport v1.0 (ADR-026) - the permanent home for one
 * machine, aggregating Identity/Lifecycle/Ownership/Warranty/PM/Quality/
 * Documents/Activity/Knowledge/Future IoT. `machineId` is the machine's
 * Serial Number (same identifier `/vehicles/[serial]` already keys on) -
 * not the `vehicles.id` UUID, since a search result can reference a
 * Tractor-IN-sheet row that hasn't synced into `vehicles` yet and would
 * have no `id` to route on. See `docs/architecture/MACHINE_PASSPORT_ARCHITECTURE.md`.
 *
 ...
 * Warranty/PM/Quality/Documents/Activity query never blocks the sections
 * above it from painting (Machine Digital Passport's "load sections
 * independently, lazy-load heavy widgets" requirement) - matching the
 * MSEAL Design Framework's Skeleton-loading guidance instead of a
 * full-page spinner.
 */
