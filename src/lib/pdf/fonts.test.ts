...
describe('shared PDF Sarabun font registration', () => {
  ...
  it('keeps Thai Sara Am as a single Unicode glyph for PDF text output', async () => {
    await ensureFontsRegistered();

    const font = Font.getFont({ fontFamily: 'Sarabun', fontStyle: 'normal', fontWeight: 'normal' }).data as unknown as LoadedSarabun;
