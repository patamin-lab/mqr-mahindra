...
### 2026-07-19 — Historic Warranty timeline date normalization
...
| Medium | Delivery / Machine Passport | Two UI panels labeled `warrantyActivatedAt` (a processing timestamp) as if it were the warranty start date | Correct underlying data, misleading label/value pairing | Delivery Record retains the explicitly labeled processing timestamp; Machine Passport now displays its authoritative Warranty Start Date from the delivery-date summary | Presentation-only; no data or logic change |
